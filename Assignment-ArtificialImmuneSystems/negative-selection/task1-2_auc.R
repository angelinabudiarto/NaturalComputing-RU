...
# Execute in terminal: 'java -jar negsel2.jar -alphabet file://english.train -self english.train -n 10 -r x -c -l < merged.test > Task1/merged_test_results_rx.txt'
# Execute in terminal: 'java -jar negsel2.jar -alphabet file://english.train -self english.train -n 10 -r x -c -l < english.test > Task1/english_test_results_rx.txt'
# Execute in terminal: 'java -jar negsel2.jar -alphabet file://english.train -self english.train -n 10 -r x -c -l < tagalog.test > Task1/tagalog_test_results_rx.txt'

merged_results <- read.table('Task1/merged_test_results_rx.txt')[,1]
english_results <- read.table('Task1/english_test_results_rx.txt')[,1]
tagalog_results <- read.table('Task1/tagalog_test_results_rx.txt')[,1]
...
plot(sort(sensitivity_auc), sort(specificity_auc), type="l", col="red",
     ...
     main=(paste('ROC-curve, n=10 r=x, score=',format(round(auc_roc_score, 2), nsmall = 2))))
